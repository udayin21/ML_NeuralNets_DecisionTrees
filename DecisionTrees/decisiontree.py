...
for j in numerical_positions:
	values = [int(i[j]) for i in list_train[1:]]
	med = statistics.median(values)
	train_medians.append(med)

for i in list_train[1:]:
	count=0
	for j in numerical_positions:
		if int(i[j])>train_medians[count]:
			i[j]=1
		else:
			i[j]=0	
		count+=1	
...
test_medians = []
for j in numerical_positions:
	values = [int(i[j]) for i in list_test[1:-1]]
	med = statistics.median(values)
...
validation_medians = []
for j in numerical_positions:
	values = [int(i[j]) for i in list_valid[1:-1]]
	med = statistics.median(values)
...
for i in list_valid[1:-1]:
	count=0
	for j in numerical_positions:
		if int(i[j])>validation_medians[count]:
			i[j]=1
		else:
			i[j]=0	
		count+=1
...
trainhelper = [i for i in range(1,len(list_train[1:])+1)]
...
test_accuracy= []
validation_accuracy = []
train_total = len(list_train[1:])
test_total = len(list_test[1:])
valid_total = len(list_valid[1:])
# print(splitonattribute(root.data[0],root.data[1],root.data[2]))
...
def makeTree(node):
	...
	if (kk[0]):
		...
		if not attribute_pick==-1:
			branches = list(set([i[attribute_pick] for i in list_train[1:]]))
			for i in branches:
