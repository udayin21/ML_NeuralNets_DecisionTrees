...
# 	outputs= []
# 	for x in X:
# 		output=calculate_output(x,outputs_neuralnet,weights_neuralnet)
# 		if (output[-1][0]>0.5):
# 			outputs.append(1)
# 		else:
# 			outputs.append(0)
...
# plot_decision_boundary(lambda x:predict(x),np.array(trainX),np.array(trainY))
...
# print('train plot')
# plot_decision_boundary(lambda x:predict(x),np.array(trainX),np.array(trainY).flatten())

print('test plot')
plot_decision_boundary(lambda x:predict(x),np.array(testX),np.array(testY).flatten())
